...
export const userStore = create<UserLoginStore>((set, get) => ({
  ...
  Get: async (id:string) => {
    try {
      const response = await api.get<UsersInterface>(`/users/user/${id}`);
      set({ item: response.data });
    } catch (err) {
      console.error(err);
    }
  },
  getCompanyId:"693c01d77717f90f375e0fc1",
  // GET todos usuários
  GetAll: async () => {
   try {  
      
      const response = await api.get<UsersInterface[]>("/users/user");
  ...
  Create: async (data) => {
    try {
      const response = await api.post<any>("/users/user/", data);
